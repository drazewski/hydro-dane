...
const CharacteristicFlows = ({ selectedStation }: Props) => {
  ...
  const rows = [
    [config.symbols[0], config.descriptions[0], characteristics.snq],
    [config.symbols[1], config.descriptions[1], characteristics.ssq],
    [config.symbols[2], config.descriptions[2], characteristics.swq],
  ] as const;
  ...
  return (
    <section className={styles.container}>
      ...
      <Table mt="md" withRowBorders className={styles.table}>
        <Table.Tbody>
          {rows.map(([symbol, description, value]) => (
            <Table.Tr key={symbol}>
